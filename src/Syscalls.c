...
BOOL FetchNtSyscallStub(_In_ ULONG ulSyscallHash, _Out_ PSYSCALL pNtSyscall) {
    ...
    if (!(pNtSyscall->ulSyscallHash = ulSyscallHash)) {
        return FALSE;
    }
...
BOOL InitSyscalls(_Out_ PSYSCALL_API pSyscallApi) {
    ...
    if (!FetchNtSyscallStub(HASHA("NtAllocateVirtualMemory"), &pSyscallApi->NtAllocateVirtualMemory)) {
        return FALSE;
    }
